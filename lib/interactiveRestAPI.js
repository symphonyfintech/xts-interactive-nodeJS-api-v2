  ...
  module.exports = class XTSInteractive {
    ...
    /**
     * getDealerPositions API of the application provides functionality of getting dealer postion details from the application
     *
     * @param {Object} reqObject request object.
     *
     * @param {string} reqObject.dayOrNet
     ...
     *  clientID of logged in dealer 
     */
    async getDealerPositions(reqObject) {
      try {
        await this.checkLoggedIn();
        let response = await request.processRequest(
          'GET',
          this.url +
          settings.restApi.dealerpositions +
          '?dayOrNet=' +
          reqObject.dayOrNet +
          '&clientID=' +
          reqObject.clientID, {
    ...
    /**
     * getDealerOrders API of the application provides functionality of getting dealer postion details from the application
     *
     * @param {Object} reqObject request object.
     *
     * @param {string} reqObject.dayOrNet
     ...
     *  clientID of logged in dealer 
     */
    async getDealerOrders(reqObject) {
      try {
        await this.checkLoggedIn();
        let response = await request.processRequest(
          'GET',
          this.url + settings.restApi.dealerorders +
          '?clientID=' + reqObject.clientID, {
            authorization: this.token
          },
          null,
          30000
    ...
    async loginWithToken(userID, token) {
      try {
        ...
        response = await request.processRequest(
          "GET", 
          this.url + settings.restApi.enums + "?userID=" + userID, {
            'authorization': token
          },
        );
        this.userID = userID;
    ...
    async hostLookup() {
      try {
        ...
        this.uniqueKey = response.result.uniqueKey;
        this.connectionString = response.result.connectionString;

        return {
          uniqueKey: response.result.uniqueKey,
          connectionString: response.result.connectionString,
        };
      } catch (error) {
        let customError = new CustomError(error.message, error.stack, error.statusCode);
        return customError;
